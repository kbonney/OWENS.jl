...
function sort_peaks(signal::AbstractArray{Float64,1}, dt = collect(1.0:length(signal)))
    slope = diff(signal)
...
function calcSF(
    ...
    topstrainout =
        zeros(length(eps_x[1, :, 1]), lencomposites_span, length(lam_in[1, :]), 9) # time, span, lam, x,y, Assumes you use zero plies for sections that aren't used
        ...
        for j_lam = 1:length(lam_in[i_station, :])
            ...
            if calculate_fatigue
                ...
                for ilayer = 1:length(lam.nply)
                    ...
                    damage_layers[ilayer] = fatigue_damage(
                        stressForFatigue,
                        SN_stress,
                        Log_SN_cycles2Fail1,
                        ultimate_strength,
