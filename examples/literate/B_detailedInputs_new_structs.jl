...
import OWENSAero
#### import PyPlot
runpath = path = "/home/klbonne/repos/owens/OWENS.jl/examples/literate" # to run locally, change to splitdir(@__FILE__)[1]
...
runpath = path = "/home/klbonne/repos/owens/OWENS.jl/examples/literate"
Inp = OWENS.MasterInput("$runpath/sampleOWENS.yml")

# Construct config structs from input
mesh_config = OWENS.MeshConfig(
    Nslices = Inp.Nslices,
    ntheta = Inp.ntheta,
    ntelem = Inp.ntelem,
    nbelem = Inp.nbelem,
    ncelem = Inp.ncelem,
    nselem = Inp.nselem,
    meshtype = "default", # or whatever is appropriate
    custommesh = nothing,
...
tower_config = OWENS.TowerConfig(
    Htwr_base = Inp.towerHeight,
    Htwr_blds = Inp.towerHeight,
    strut_twr_mountpoint = [0.2, 0.8],
    strut_bld_mountpoint = [0.2, 0.8],
    joint_type = 0,
    c_mount_ratio = 0.05,
    angularOffset = 0.0,
    NuMad_geom_xlscsv_file_twr = "$(runpath)$(Inp.NuMad_geom_xlscsv_file_twr)",
    NuMad_mat_xlscsv_file_twr = "$(runpath)$(Inp.NuMad_mat_xlscsv_file_twr)"
)


blade_config = OWENS.BladeConfig(
    B = Inp.Nbld,
    H = Inp.Blade_Height,
    R = Inp.Blade_Radius,
    shapeZ = collect(LinRange(0, Inp.Blade_Height, Inp.Nslices+1)),
    shapeX = Inp.Blade_Radius .* (1.0 .- 4.0 .* (collect(LinRange(0, Inp.Blade_Height, Inp.Nslices+1)) ./ Inp.Blade_Height .- .5).^2),
    shapeY = zeros(Inp.Nslices+1),
    NuMad_geom_xlscsv_file_bld = "$(runpath)$(Inp.NuMad_geom_xlscsv_file_bld)",
    NuMad_mat_xlscsv_file_bld = "$(runpath)$(Inp.NuMad_mat_xlscsv_file_bld)"
)

material_config = OWENS.MaterialConfig(
    stack_layers_bld = nothing, # or Inp.stack_layers_bld if present
    stack_layers_scale = ones(Inp.Nslices+1),
    chord_scale = ones(Inp.Nslices+1),
    thickness_scale = ones(Inp.Nslices+1)
)

aero_config = OWENS.AeroConfig(
    rho = Inp.rho,
    mu = 1.7894e-5,
    RPM = Inp.RPM,
    Vinf = Inp.Vinf,
...
mesh_outputs, blade_props, tower_props, strut_props, aero_props, omega, tsr = OWENS.setupOWENS(
    mesh_config, tower_config, blade_config, material_config, aero_config
